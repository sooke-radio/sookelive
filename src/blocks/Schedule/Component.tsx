...
export const ScheduleBlock: React.FC<Props> = async ({ shows = [], allShows = false, title , description = "" }) => {
                  ...
                  {daySchedule.map((entry, entryIndex) => (
                    <div key={entryIndex} className="space-y-1">
                      ...
                      <div className="flex border-b border-1 border-dotted">
                        <span className="text-sm min-w-48">{formatTime(entry.startTime)} - {formatTime(entry.endTime)}</span>
                        <div className="text-sm font-bold"><a href={`/shows/${entry.slug}`}>{entry.showName}</a></div>
                     </div>
